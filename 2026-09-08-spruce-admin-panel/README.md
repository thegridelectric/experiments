...
**Code under test.** gridworks-scada `jm/spruce-unlimbo` at `c8555abe`
(rung 1 plus the 09-08 dev-broker fixes: cycler and command-node rows
...
`c8555abe`, checked on the laptop with `sema_to_dc.load_layout` before
the window):
...
- 10:22:03 window scada up (`c8555abe`); 10:22:06 boot vdc cycle,
