#!/usr/bin/env bash
# Swap beech between the deployed scada and a window scada on the (dist-010v sweep rung; the
# krida witness rung's script with this folder's instances) unlimbo checkout, from the laptop, in one word each. The beech copy of
...
EXP=experiments/2026-09-12-beech-dist-010v-sweep
...
case "${1:-}" in
  on)
    ...
    echo "window scada up (bound $MIN min); now, ON THE BOX: see README Protocol step 2 (dist_sweep.py from ~/experiments)"
    ;;
  off)
