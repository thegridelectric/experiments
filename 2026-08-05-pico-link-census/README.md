...
All data was GENERATED by the census run itself: live observation of
each house LAN's neighbor table over ssh — in no store, so a re-run
produces a NEW census (an offline pico is invisible; re-run to catch
stragglers). Nothing touched the running system. No data files are
committed: the finding is negative (OUI cannot discriminate link
type), so the README narrative is the record and the raw tables are
reproduced on demand:

    uv run python pico_link_census.py

- `pico_link_census.py` — the reproducer (ssh + `ip neigh`, OUI
  classification into typed `Neighbor` records). MACs are
  `mac.address`-shaped; that format enters the vendored snapshot with
  the next regen (see the module docstring). HwUid is the RP2040
  flash serial (`machine.unique_id()[-6:]`), not MAC-derived.
