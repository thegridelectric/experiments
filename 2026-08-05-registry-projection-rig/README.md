...
## Manifest & experimental method

All evidence was GENERATED by the live rig run: broker traffic,
projection rows, and the ear's local retry cache — observed in place,
in no store this folder can re-pull; a re-run stands the rig up again
from the Setup pins above and produces a new dataset. No data files
are committed; the Found section is the record.

- `rig_reparent.py` — the leg-3 MarketMaker publisher (legs 2 and 4
  are the one-line commands above). Runs from the grid-node-registry
  root in gnr's environment (excluded from this repo's pyright gate);
  speaks gnr's vendored snapshot both ways — commands constructed
  through it, the forest broadcast decoded through it with
